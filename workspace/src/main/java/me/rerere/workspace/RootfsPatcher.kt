...
class RootfsPatcher {
    fun patch(
        ...
        ensureAlpineApkRepositories(linuxDir)
    ...
    private fun ensureRootfsDns(
        ...
        val needsRewrite = when {
            Files.isSymbolicLink(resolvConf.toPath()) -> true
            !resolvConf.exists() -> true
            !resolvConf.isFile -> true
            resolvConf.length() < 10 -> true
            else -> {
                val content = runCatching { resolvConf.readText() }.getOrDefault("")
                !content.contains("nameserver 1.1.1.1") && !content.contains("nameserver 8.8.8.8")
            }
        }
        ...
        resolvConf.writeText(
            buildString {
                appendLine("# Generated by BIT workspace.")
                servers.forEach { appendLine("nameserver $it") }
                appendLine("options timeout:2 attempts:3")
            }
        )
    }
    ...
    private fun ensureAlpineApkRepositories(linuxDir: File) {
        val etcDir = File(linuxDir, "etc/apk")
        if (!etcDir.isDirectory) return
        val repoFile = File(etcDir, "repositories")
        if (repoFile.exists()) {
            val lines = repoFile.readLines()
            val newLines = lines.map { line ->
                val trimmed = line.trim()
                if (trimmed.startsWith("#") && trimmed.contains("alpinelinux.org")) {
                    trimmed.removePrefix("#").trim()
                } else {
                    line
                }
            }.toMutableList()
            if (newLines.none { it.contains("/community") }) {
                val mainLine = newLines.firstOrNull { it.contains("/main") }
                if (mainLine != null) {
                    newLines.add(mainLine.replace("/main", "/community"))
                }
            }

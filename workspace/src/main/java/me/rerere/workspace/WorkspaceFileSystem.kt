...
class WorkspaceFileSystem(
    ...
    private fun File.toEntry(root: File): WorkspaceFileEntry {
        val isSymlink = Files.isSymbolicLink(toPath())
        val isDir = isDirectory || (isSymlink && runCatching { Files.isDirectory(toPath()) }.getOrDefault(false))
        return WorkspaceFileEntry(
            path = relativePath(root),
            name = name,
            isDirectory = isDir,
            sizeBytes = if (!isDir) length() else 0L,

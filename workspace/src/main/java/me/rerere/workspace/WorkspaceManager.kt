...
fun File.hasUsableRootfs(): Boolean {
    ...
    return candidates.any { rel ->
        val file = File(this, rel)
...
fun File.rootfsInterpreter(): String? {
    ...
    for (rel in candidates) {
        var file = File(this, rel)
        if (Files.isSymbolicLink(file.toPath())) {
            val linkTarget = runCatching { Files.readSymbolicLink(file.toPath()).toString() }.getOrNull()
            if (linkTarget != null) {
                file = if (linkTarget.startsWith("/")) {
                    File(this, linkTarget.trimStart('/'))
                } else {
                    File(file.parentFile ?: this, linkTarget)
                }
            }
        }
        if (file.exists() && file.isFile) {
            val interp = runCatching { file.elfInterpreter() }.getOrNull()
            if (!interp.isNullOrBlank()) return interp
        }
    }

    val libDirs = listOf(
        "lib", "lib64", "usr/lib", "usr/lib64",
        "lib/aarch64-linux-gnu", "lib/x86_64-linux-gnu", "lib/arm-linux-gnueabihf"
    )
    for (dirName in libDirs) {
        val dir = File(this, dirName)
        if (dir.isDirectory) {
            val ld = dir.listFiles()?.firstOrNull { f ->
                (f.name.startsWith("ld-linux") || f.name.startsWith("ld-musl")) && !f.name.endsWith(".bak")
            }
            if (ld != null) {
                return "/$dirName/${ld.name}"
            }
        }
    }
    return null
    ...
    for (rel in candidates) {
        ...
        val result = runCatching {
            file.inputStream().use { input ->
                ...
                WorkspaceRootfsArchitecture.fromElfMachine(machine)
            }
        }.getOrNull()

...
import com.bit.models.table_schema.RagSourceType
...
class RagRepository(
    ...
    suspend fun createRagFromChat(
        ...
        description: String,
        chatId: String,
        messages: List<com.bit.models.messages.Messages>,
        graph: NeuronGraph,
        domain: String = "general",
        tags: List<String> = emptyList()
    ): Result<InstalledRag> = withContext(Dispatchers.IO) {
    ...
    suspend fun queryAllLoadedGraphs(
        query: String,
        topK: Int = 5
    ): List<Pair<InstalledRag, List<com.bit.neuron_example.QueryResult>>> = withContext(Dispatchers.IO) {
        val results = mutableListOf<Pair<InstalledRag, List<com.bit.neuron_example.QueryResult>>>()

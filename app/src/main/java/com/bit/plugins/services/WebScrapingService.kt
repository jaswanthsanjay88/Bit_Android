...
class WebScrapingService {
    ...
    private val baseClient = CurlImpersonateHelper.getClient(CurlImpersonateHelper.BrowserType.CHROME, timeoutSeconds = 30)
        ...
        repeat(MAX_RETRIES) { attempt ->
            try {
                ...
                client.newCall(request).execute().use { response ->
                    ...
                    // Sanitize extracted content and apply length limit
    ...
    /**
     * Detect charset from content type header with fallbacks
     */
    private fun detectCharset(contentType: String?): Charset {
        return try {
            // Try Content-Type header first
            contentType?.let {
                val charsetMatch = Regex("charset=([^;\\s]+)", RegexOption.IGNORE_CASE).find(it)
                charsetMatch?.groupValues?.get(1)?.let { cs ->
                    try {
                        return Charset.forName(cs.trim('"'))
                    } catch (e: Exception) {
                        // Charset name is invalid, use UTF-8
                    }
                }
            }
            // Fallback to UTF-8
    ...
    /**
     * Sanitize text content by removing control characters and invalid Unicode sequences
     */
    private fun sanitizeContent(content: String): String {
        return content
            .replace(Regex("[\\p{Cc}\\p{Cn}]"), " ") // Remove control and unassigned chars
            .replace(Regex("\\s+"), " ") // Collapse multiple whitespace
            .replace(Regex("^\\s+|\\s+$"), "") // Trim
            .filter { it.isValidChar() } // Keep only valid characters
    }

    /**
     * Check if a character is valid for display
     */
    private fun Char.isValidChar(): Boolean {
        return when {
            this.isWhitespace() -> true // Allow whitespace
            this.code < 32 && this !in '\t'..'\n' && this != '\r' -> false // Remove control chars except tab/newline
            this.code in 0xFFFE..0xFFFF -> false // Invalid unicode range
            else -> true
        }
    }
    ...
    private fun extractElementContent(element: Element): String {
        return buildString {
            ...
            element.select("h1, h2, h3, h4, h5, h6").forEach {
                val text = sanitizeContent(it.text().trim())
    ...
    private fun extractTable(table: Element): String {
        return buildString {
            table.select("tr").forEach { row ->
                ...
                val sanitizedCells = cells.map { sanitizeContent(it.text().trim()) }
                appendLine(sanitizedCells.joinToString(" | "))
            }
        }
    }
    ...
    private fun extractMainContent(document: Document): String {
        ...
        return buildString {
            ...
            for (element in elements) {
                val text = sanitizeContent(element.text().trim())
